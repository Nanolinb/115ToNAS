...
async def scan_file(path: Path, lib_type: str, force: bool = False):
    ...
    # TMDB 缺简介/题材/年份 → subhd 一把抓兜底（豆瓣补简介）
    if not values["overview"] or not values["genres"] or not values["year"]:
        from . import overview as _ov
        m = await _ov.fetch_meta(values["name_cn"] or values["title"], values["year"])
        if m:
            values["overview"] = values["overview"] or m.get("overview")
            values["genres"] = values["genres"] or m.get("genres")
            values["year"] = values["year"] or m.get("year")
...
async def rematch(media_id: int, tmdb_id: int | None = None,
    ...
    if not meta.get("overview") or not meta.get("genres"):
        ...
        from . import overview as _ov
        m = await _ov.fetch_meta(
            meta.get("name_cn") or meta.get("title") or row["title"],
            meta.get("year") or row["year"])
        if m:
            meta["overview"] = meta.get("overview") or m.get("overview")
            meta["genres"] = meta.get("genres") or m.get("genres")
